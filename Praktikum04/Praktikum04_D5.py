...
def classify_gesture(hand):
    # hand["lmList"] berisi 21 titik (x,y,x) dalam piksel saat flipType=True
    lm = hand["lmList"]
    ...
    if dist(index_tip, wrist) > 180 and dist(middle_tip, wrist) > 180 and dist (ring_tip, wrist) < 160 and dist(pinky_tip, wrist) < 160:
        return "SCISSORS"
    return "UNKNOWN"
...
detector = HandDetector(staticMode=False, maxHands=1, modelComplexity=1, detectionCon=0.5
                        , minTrackCon=0.5)
